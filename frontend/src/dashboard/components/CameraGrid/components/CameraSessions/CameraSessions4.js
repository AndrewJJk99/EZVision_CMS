...
export default function CameraSessions4({
  activeCameraCount = 1,
  ...
  const setupWebSocketHandlers = React.useCallback((websocket) => {
    websocket.onmessage = (messageEvent) => {
      ...
      const canvas = canvasRef.current;
      if (!canvas) {
        console.warn('[CameraSessions4] Canvas ref is null');
        return;
      }
      ...
      if (messageEvent.data instanceof Blob) {
        ...
        console.warn('[CameraSessions4] Unknown data type received:', typeof messageEvent.data);
        return;
      }
      ...
      img.onerror = (error) => {
        console.error('[CameraSessions4] Error loading image:', error);
        URL.revokeObjectURL(imageUrl);
      };
  ...
  const handleStreamingToggle = React.useCallback(async (event) => {
    ...
    if (shouldStream) {
      ...
      try {
        ...
        newWs.onopen = () => {
          console.log(`[CameraSessions4] Manual WebSocket connected for camera ${cameraId}`);
          wsRef.current = newWs;
          setIsStreaming(true);
        ...
        newWs.onerror = (error) => {
          console.error(`[CameraSessions4] Manual WebSocket error for camera ${cameraId}:`, error);
          setIsStreaming(false);
        ...
        newWs.onclose = (event) => {
          console.log(`[CameraSessions4] Manual WebSocket closed for camera ${cameraId}, code: ${event.code}`);
          wsRef.current = null;
          setIsStreaming(false);
          autoStartCheckedRef.current = false;
          manualToggleRef.current = false;
        ...
        console.error(`[CameraSessions4] Error starting manual streaming:`, error);
        setIsStreaming(false);
        manualToggleRef.current = false;
      }
    } else {
  ...
  const checkAndStartStreaming = React.useCallback(async () => {
    ...
    try {
      console.log(`[CameraSessions4] Checking camera ${cameraId} status for auto-start`);
      const statusResponse = await getCameraStatus(cameraIdBackend);
      const status = statusResponse?.status;
      
      if (status && status.is_running) {
        console.log(`[CameraSessions4] Camera ${cameraId} is running, starting auto-streaming`);
        autoStartCheckedRef.current = true;
        ...
        newWs.onopen = () => {
          console.log(`[CameraSessions4] Auto-WebSocket connected for camera ${cameraId}`);
          wsRef.current = newWs;
          setIsStreaming(true);
        ...
        newWs.onerror = (error) => {
          console.error(`[CameraSessions4] Auto-WebSocket error for camera ${cameraId}:`, error);
          autoStartCheckedRef.current = false;
        };
        
        newWs.onclose = (event) => {
          console.log(`[CameraSessions4] Auto-WebSocket closed for camera ${cameraId}, code: ${event.code}`);
          wsRef.current = null;
          setIsStreaming(false);
          autoStartCheckedRef.current = false;
        };
      } else {
        console.log(`[CameraSessions4] Camera ${cameraId} is not running, skipping auto-start`);
      }
    } catch (error) {
      console.error(`[CameraSessions4] Error checking camera status:`, error);
    }
  ...
  // 스트리밍 시작 이벤트 리스너
  React.useEffect(() => {
    console.log('[CameraSessions4] Component mounted, setting up event listeners');
    
    const handleStreamingStart = (event) => {
      console.log('[CameraSessions4] streamingStart event received:', event.detail);
      
      // 카메라 4번만 처리
      if (event.detail.cameraId !== cameraId) {
        console.log(`[CameraSessions4] Ignoring event for camera ${event.detail.cameraId}, waiting for camera ${cameraId}`);
        return;
      }

      console.log(`[CameraSessions4] Setting up WebSocket for camera ${cameraId}`);
      ...
      console.log(`[CameraSessions4] isStreaming set to true for camera ${cameraId}`);
      ...
      websocket.onerror = (error) => {
        console.error(`[CameraSessions4] WebSocket error for camera ${cameraId}:`, error);
        setIsStreaming(false);
      };

      websocket.onclose = (event) => {
        console.log(`[CameraSessions4] WebSocket closed for camera ${cameraId}, code: ${event.code}, reason: ${event.reason}`);
        setIsStreaming(false);
        wsRef.current = null;
      };
    };
    ...
    const handleStreamingStop = (event) => {
      console.log('[CameraSessions4] streamingStop event received:', event.detail);
      
      // detail이 없거나 cameraId가 없으면 모든 카메라에 대해 중지 (언마운트 시 등)
      if (!event.detail || !event.detail.cameraId) {
        console.log('[CameraSessions4] Streaming stop event without cameraId, stopping all');
        if (wsRef.current) {
          wsRef.current.close();
          wsRef.current = null;
        }
      ...
      // 카메라 4번만 처리
      if (event.detail.cameraId !== cameraId) {
        console.log(`[CameraSessions4] Ignoring stop event for camera ${event.detail.cameraId}, waiting for camera ${cameraId}`);
        return;
      }

      console.log(`[CameraSessions4] Stopping streaming for camera ${cameraId}`);
      if (wsRef.current) {
        wsRef.current.close();
        wsRef.current = null;
      }
      ...
      <CardContent sx={{ p: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.5, flexShrink: 0, px: 0.5 }}>
          <Typography variant="subtitle2" component="div" sx={{ fontWeight: 600 }}>
            Camera 4 {calibrationActive ? '(Calibration)' : ''}
          </Typography>
          {showControls && (
            <FormControlLabel
              control={
