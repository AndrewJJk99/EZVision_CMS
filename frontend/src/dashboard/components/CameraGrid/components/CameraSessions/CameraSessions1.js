...
export default function CameraSessions1({
  activeCameraCount = 1,
  ...
  // 캘리브레이션 중 HTTP 프리뷰만 사용 — WebSocket get_frame 경쟁 방지
  React.useEffect(() => {
  ...
  const setupWebSocketHandlers = React.useCallback((websocket) => {
    websocket.onmessage = (messageEvent) => {
      ...
      const canvas = canvasRef.current;
      if (!canvas) {
        console.warn('[CameraSessions1] Canvas ref is null');
        return;
      }
      ...
      if (messageEvent.data instanceof Blob) {
        ...
        console.warn('[CameraSessions1] Unknown data type received:', typeof messageEvent.data);
        return;
      }
      ...
      img.onerror = (error) => {
        console.error('[CameraSessions1] Error loading image:', error);
        URL.revokeObjectURL(imageUrl);
      };
  ...
  const handleStreamingToggle = React.useCallback(async (event) => {
    ...
    if (shouldStream) {
      try {
        ...
        newWs.onopen = () => {
          console.log(`[CameraSessions1] Manual WebSocket connected for camera ${cameraId}`);
          wsRef.current = newWs;
          setIsStreaming(true);
        ...
        newWs.onerror = (error) => {
          console.error(`[CameraSessions1] Manual WebSocket error for camera ${cameraId}:`, error);
          setIsStreaming(false);
        ...
        newWs.onclose = (event) => {
          console.log(`[CameraSessions1] Manual WebSocket closed for camera ${cameraId}, code: ${event.code}`);
          wsRef.current = null;
          setIsStreaming(false);
          autoStartCheckedRef.current = false;
          manualToggleRef.current = false;
        ...
        console.error(`[CameraSessions1] Error starting manual streaming:`, error);
        setIsStreaming(false);
        manualToggleRef.current = false;
      }
    } else {
  ...
  const checkAndStartStreaming = React.useCallback(async () => {
    ...
    try {
      console.log(`[CameraSessions1] Checking camera ${cameraId} status for auto-start`);
      const statusResponse = await getCameraStatus(cameraIdBackend);
      const status = statusResponse?.status;
      
      if (status && status.is_running) {
        console.log(`[CameraSessions1] Camera ${cameraId} is running, starting auto-streaming`);
        autoStartCheckedRef.current = true;
        
        const newWs = new WebSocket(getCameraWsUrl(cameraIdBackend));
        
        newWs.onopen = () => {
          console.log(`[CameraSessions1] Auto-WebSocket connected for camera ${cameraId}`);
          wsRef.current = newWs;
          setIsStreaming(true);
          newWs.binaryType = 'blob';
          setupWebSocketHandlers(newWs);
        ...
        newWs.onerror = (error) => {
          console.error(`[CameraSessions1] Auto-WebSocket error for camera ${cameraId}:`, error);
          autoStartCheckedRef.current = false;
        };
        
        newWs.onclose = (event) => {
          console.log(`[CameraSessions1] Auto-WebSocket closed for camera ${cameraId}, code: ${event.code}`);
          wsRef.current = null;
          setIsStreaming(false);
          autoStartCheckedRef.current = false;
        };
      } else {
        console.log(`[CameraSessions1] Camera ${cameraId} is not running, skipping auto-start`);
      }
    } catch (error) {
      console.error(`[CameraSessions1] Error checking camera status:`, error);
    }
  ...
  React.useEffect(() => {
    ...
    const handleStreamingStart = (event) => {
      console.log('[CameraSessions1] streamingStart event received:', event.detail);
      
      if (event.detail.cameraId !== cameraId) {
        console.log(`[CameraSessions1] Ignoring event for camera ${event.detail.cameraId}, waiting for camera ${cameraId}`);
        return;
      }

      console.log(`[CameraSessions1] Setting up WebSocket for camera ${cameraId}`);
      ...
      setIsStreaming(true);
      autoStartCheckedRef.current = true;
      console.log(`[CameraSessions1] isStreaming set to true for camera ${cameraId}`);

      websocket.binaryType = 'blob';
      setupWebSocketHandlers(websocket);

      websocket.onerror = (error) => {
        console.error(`[CameraSessions1] WebSocket error for camera ${cameraId}:`, error);
        setIsStreaming(false);
      };

      websocket.onclose = (event) => {
        console.log(`[CameraSessions1] WebSocket closed for camera ${cameraId}, code: ${event.code}, reason: ${event.reason}`);
        setIsStreaming(false);
        wsRef.current = null;
      };
    };
    ...
    const handleStreamingStop = (event) => {
      console.log('[CameraSessions1] streamingStop event received:', event.detail);
      
      if (!event.detail || !event.detail.cameraId) {
        console.log('[CameraSessions1] Streaming stop event without cameraId, stopping all');
        if (wsRef.current) {
          wsRef.current.close();
          wsRef.current = null;
        }
      ...
      if (event.detail.cameraId !== cameraId) {
        console.log(`[CameraSessions1] Ignoring stop event for camera ${event.detail.cameraId}, waiting for camera ${cameraId}`);
        return;
      }

      console.log(`[CameraSessions1] Stopping streaming for camera ${cameraId}`);
      if (wsRef.current) {
        wsRef.current.close();
        wsRef.current = null;
      }
